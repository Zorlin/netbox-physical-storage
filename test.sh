...
test_netbox_unit_tests() {
  ...
  $doco run --rm netbox python manage.py makemigrations netbox_physical_storage --check
  $doco run --rm netbox python manage.py test netbox_physical_storage -v 2
}
...
test_cleanup() {
  ...
  $doco down -v
  $doco rm -fsv
  docker image rm docker.io/library/netbox-physical-storage-netbox || echo ''
}
