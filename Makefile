PLUGIN_NAME=netbox_physical_storage
REPO_PATH=/opt/netbox/netbox/netbox-physical-storage
...
#cleanup:
#	-docker-compose -p netbox-physical-storage_devcontainer rm -fv
...
nbshell:
	...
	from netbox_physical_storage.models import *
...
example_initializers:
	-${VENV_PY_PATH} ${NETBOX_MANAGE_PATH}/manage.py copy_initializers_examples --path /opt/netbox/netbox/netbox-physical-storage/.devcontainer/initializers

.PHONY: load_initializers ## Run initializers
load_initializers:
	-${VENV_PY_PATH} ${NETBOX_MANAGE_PATH}/manage.py load_initializer_data  --path /opt/netbox/netbox/netbox-physical-storage/.devcontainer/initializers
...
test: setup
	...
	coverage run --source "netbox_physical_storage" ${NETBOX_MANAGE_PATH}/manage.py test ${PLUGIN_NAME} -v 2
